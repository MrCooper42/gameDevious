...
export var ProfileEditComponent = (function () {
    ...
    ProfileEditComponent.prototype.refreshWorks = function () {
        ...
        this.returnUser().subscribe(function (res) {
            console.log(_this.user, "refreshWorks");
            _this.user = res;
        });
        this.showWork = !this.showWork;
    ...
    ProfileEditComponent.prototype.submitEdu = function () {
        var _this = this;
        var education = this.eduForm.value;
        this.profileService.addEdu(this.useJwtHelper()._id, education)
            .subscribe(function (data) {
            _this.user.profile.education = data.education;
            _this.router.navigateByUrl('/profile');
        }, function (error) { return console.error(error); });
        this.eduForm.reset();
        this.showEduForm = false;
    };
    ProfileEditComponent.prototype.submitXp = function () {
        var _this = this;
        var experience = this.xpForm.value;
        this.profileService.addExp(this.useJwtHelper()._id, experience)
            .subscribe(function (data) {
            _this.user.profile.experience = data.experience;
            _this.router.navigateByUrl('/profile');
        }, function (error) { return console.error(error); });
        this.xpForm.reset();
    ...
    ProfileEditComponent.prototype.workAvatar = function (id) {
        for (var i = 0; i < this.user.profile.works.length; i++) {
            if (this.user.profile.works[i]._id = id) {
                var avatar = this.user.profile.works[i].path;
                if (avatar == undefined) {
                    return false;
                }
